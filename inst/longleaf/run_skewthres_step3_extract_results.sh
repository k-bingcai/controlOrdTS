...
step_3_iden="standard"

# Output location
out_loc_str=${SIM_DIRECTORY}"/"${filename} 

# Clean out slurm folder
if [ -d ${out_loc_str}"/slurm/skewthres-step-3-"${step_3_iden} ]
then 
	pushd ${out_loc_str}"/slurm/skewthres-step-3-"${step_3_iden} >> /dev/null 2>&1
	mkdir -p archive
...
# Create SLURM file 
Rscript R/skewthres-step-3-extract-results-SLURM.R --models_json_file ${mod_file} \
    --gen_json_file ${gen_file} \
    --sim_dir ${out_loc_str} \
    --code_dir ${curr_dir}"/R" \
    --logs_suffix ${step_3_iden} || exit 1

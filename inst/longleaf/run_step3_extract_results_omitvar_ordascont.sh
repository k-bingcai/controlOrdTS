...
then 
	pushd ${out_loc_str}"/slurm/step-3-"${step_3_iden} >> /dev/null 2>&1
	mkdir -p archive
...
# Create SLURM file 
Rscript R/step-3-extract-results-omitvar-SLURM.R --models_json_file ${mod_file} \

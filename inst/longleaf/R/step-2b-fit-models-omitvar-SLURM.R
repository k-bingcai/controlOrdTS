...
slurm_out       <- paste0(args$sim_dir, "/slurm/step-2b-", args$logs_suffix)
models_dir      <- paste0(args$sim_dir, "/models")
logs_dir        <- paste0(args$sim_dir, "/logs/step-2b-", args$logs_suffix)
...
code_to_run <- paste(args$code_dir, "step-2b-fit-models-omitvar.R", sep = "/")
run_cmd <- paste("
# Command to run 
...
for (cond_i in 1:nrow(conditions_df)) {
    ...
    this_row_omit_var       <- paste0("omit_V", args$omit_var_num)
    this_row_name           <- paste(this_row_model_num, this_row_skeleton,
                                     this_row_ord_num, this_row_omit_var, sep = "_")
    ...
    log.o <- paste0("#SBATCH --output=", logs_dir, "/slurm-step-2b-", this_row_name, ".stdout")
    log.e <- paste0("#SBATCH --error=", logs_dir, "/slurm-step-2b-", this_row_name, ".stderr")
    ...
    cmd_string <- paste(cmd_string, "--omit_var_num", args$omit_var_num, sep = " ")

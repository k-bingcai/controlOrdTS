...
parser$add_argument('--skeleton_seed', required = TRUE, help = "Seed for skeleton graph generator")
...
# Create mod_arg_list
mod_i_arg_list    <- list()
mod_i_seed        <- suppressWarnings(as.integer(args$skeleton_seed))
if (!is.na(mod_i_seed)) {
...
for (mod_ij in 1:num_random) {
    ...
    gen_ij_model   <- controlOrdTS::create_sim_obj(phi.func = get(mod_i_func),
                                                   ...
                                                   skewed_thres = TRUE)   # Default is FALSE

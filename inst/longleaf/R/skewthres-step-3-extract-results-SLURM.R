...
slurm_out       <- paste0(args$sim_dir, "/slurm/skewthres-step-3-", args$logs_suffix)
models_dir      <- paste0(args$sim_dir, "/skewthres-models")
logs_dir        <- paste0(args$sim_dir, "/logs/skewthres-step-3-", args$logs_suffix)
...
code_to_run <- paste(args$code_dir, "skewthres-step-3-extract-results.R", sep = "/")
run_cmd <- paste("
# Command to run 
...
for (cond_i in 1:nrow(conditions_df)) {
    ...
    file_conn <- file(paste0(slurm_out, "/SLURM_extract_results_", this_row_name, ".slurm"))

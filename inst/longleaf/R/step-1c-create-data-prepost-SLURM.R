...
slurm_out       <- paste0(args$sim_dir, "/slurm/step-1c")
models_dir      <- paste0(args$sim_dir, "/models")
logs_dir        <- paste0(args$sim_dir, "/logs/step-1c")
...
code_to_run <- paste(args$code_dir, "step-1c-create-data-prepost.R", sep = "/")
run_cmd <- paste("
# Command to run 
...
for (mod_i in 1:length(model_json_list$models)) {
    ...
    log.o <- paste0("#SBATCH --output=", logs_dir, "/slurm-step-1c-", mod_i_name, ".stdout")
    log.e <- paste0("#SBATCH --error=", logs_dir, "/slurm-step-1c-", mod_i_name, ".stderr")
    ...
    file_conn <- file(paste0(slurm_out, "/SLURM_create_data_prepost_", mod_i_name, ".slurm"))

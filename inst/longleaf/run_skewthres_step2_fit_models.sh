...
step_2_iden="standard"

# Output location
out_loc_str=${SIM_DIRECTORY}"/"${filename} 

# Clean out slurm folder
if [ -d ${out_loc_str}"/slurm/skewthres-step-2-"${step_2_iden} ]
then 
	pushd ${out_loc_str}"/slurm/skewthres-step-2-"${step_2_iden} >> /dev/null 2>&1
	mkdir -p archive
...
# Create SLURM file 
Rscript R/skewthres-step-2-fit-models-SLURM.R --models_json_file ${mod_file} \

...
extract_skew <- function(sim_dir, numord) {
  ...
  for (mod_i in mod_files) {
    ...
    if (numord == 7) {
      ...
      sub_thres <- function(x) {c(x[2], x[3], x[4], x[5])}
    } else if (numord == 3) {
      sub_thres <- function(x) {c(x[3], x[4])}
    } else {
      stop("Invalid numord.")
    }
...
warning("Might be better to use either raw data distribution or unsummed_quantiles to compute skew")
warning("The function used is hard-coded and is different from the non-skewed case")
warning("Check which model is being used")

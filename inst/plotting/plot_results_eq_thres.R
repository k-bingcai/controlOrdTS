...
plot_diff <- function(column, data_to_plot) {
  ...
  # Plot title
  if (data_to_plot$skeleton == "bringmann_2017_dataset1_eq_thres") {
    plt_title <- "Bringmann's (2016) 6-Node System"
  } else if (data_to_plot$skeleton == "bringmann_2017_dataset2_eq_thres") {
    plt_title <- "Bringmann's (2016) 10-Node System"
  } else if (data_to_plot$skeleton == "scale_free_seeded_eq_thres") {
  ...
  if (column == "est_correct_max_node") {
    addlabel <- paste(plt_title, "  [P(Max Node Selected)]", "  [EqThres]", sep = "")
  } else if (column == "est_max_in_top20p_nodes") {
    addlabel <- paste(plt_title, "  P(Top 20% Node Selected)", "  [EqThres]", sep = "")
  } else if (column == "gramian_sp_rank_corr") {
    addlabel <- latex2exp::TeX(paste(plt_title,
                                     "\\  [$\\rho_{\\ true, estimated}] \\ \\  [EqThres]$",
                                     sep = ""))
  } else {
...
plot_bars <- function(column, data_to_plot, log_y = FALSE, ordascont_val = FALSE) {
  ...
  # Plot title
  if (data_to_plot$skeleton == "bringmann_2017_dataset1_eq_thres") {
    plt_title <- "Bringmann's (2016) 6-Node System  [EqThres]"
  } else if (data_to_plot$skeleton == "bringmann_2017_dataset2_eq_thres") {
    plt_title <- "Bringmann's (2016) 10-Node System  [EqThres]"
  } else if (data_to_plot$skeleton == "scale_free_seeded_eq_thres") {
    plt_title <- "Scale Free Network  [EqThres]"
  } else {
    stop("Invalid plot title")
  }
